...
- MCP Interface Adapter：作为 Interfaces 层协议适配，通过 FastMCP/Streamable HTTP 暴露检索与状态 tools，复用 Application 层检索服务和健康检查服务，不直接访问 Infrastructure；N-KB 只支持 MCP Streamable HTTP transport，不支持旧版 SSE transport。
